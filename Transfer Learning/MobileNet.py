...
mobile = MobileNet(input_shape = (128, 128, 3),
                        include_top = False,
                        weights = 'imagenet')
...
filepath = root_path + '/Model/MobileNet for Dataset6/model-{epoch:02d}-{val_accuracy:.2f}.h5'
model_checkpoint = ModelCheckpoint(filepath, monitor='val_accuracy', verbose=1, save_best_only=True, mode='auto')

callbacks = [model_checkpoint]

# Add more layers
last_output = mobile.output
...
model=tf.keras.Model(inputs = mobile.input, outputs = x)
...
model_path = os.path.join(root_path, 'Model/MobileNet for Dataset6')
weight_path = model_path + '/MobileNet_model_accuracy_' + str(train_acc).replace('.', ',') + '%.h5'     #e.g. ../Model/model_accuracy_90%.h5
json_path = model_path + '/MobileNet_model_accuracy_' + str(train_acc).replace('.', ',') + '%.json'
